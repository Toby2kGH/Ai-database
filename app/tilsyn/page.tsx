...
const BRUKSMELDING_STATUS_META: Record<
  string,
  { etikett: string; farge: string; ikon: React.ReactNode }
> = {
  ny: {
    etikett: "Ny",
    farge: "bg-blue-50 text-blue-700 border-blue-200",
    ikon: <ClipboardList size={12} />,
  },
  "til-oppfolging": {
    etikett: "Til oppfølging",
    farge: "bg-amber-50 text-amber-700 border-amber-200",
    ikon: <Clock size={12} />,
  },
  godkjent: {
    etikett: "Godkjent",
    farge: "bg-green-50 text-green-700 border-green-200",
    ikon: <CheckCircle2 size={12} />,
  },
  avvist: {
    etikett: "Avvist",
    farge: "bg-red-50 text-red-700 border-red-200",
    ikon: <XOctagon size={12} />,
  },
};
...
export default function TilsynPage() {
    ...
    switch (aktivtFilter) {
      ...
      case "off-label":
        return MOCKDATA_LOSNINGER.filter(
          (l) =>
            l.godkjenningsStatus === "delvis" ||
            l.godkjenningsStatus === "ikke-godkjent"
        );
      case "mangler-dpia":
  ...
  const varsler = {
    ...
    off_label: MOCKDATA_LOSNINGER.filter(
      (l) =>
        l.godkjenningsStatus === "delvis" ||
        l.godkjenningsStatus === "ikke-godkjent"
    ).length,
    ...
    hoy_risiko: MOCKDATA_LOSNINGER.filter((l) =>
      ["hoy-medisinsk", "hoy-annex3"].includes(l.aiActRisiko)
    ).length,
  };

  const bruksVarsler = {
    ny: MOCKDATA_BRUKSMELDINGER.filter((b) => b.status === "ny").length,
    til_oppfolging: MOCKDATA_BRUKSMELDINGER.filter(
      (b) => b.status === "til-oppfolging"
    ).length,
    avvist: MOCKDATA_BRUKSMELDINGER.filter((b) => b.status === "avvist").length,
    off_label: MOCKDATA_BRUKSMELDINGER.filter(
      (b) => b.erInnenforGodkjenning !== "ja"
    ).length,
  };
      ...
      <div className="max-w-xl mx-auto px-4 py-20 text-center">
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-8">
          ...
          <p className="text-sm text-gray-600 mb-4">
            Tilsynsvisningen er kun tilgjengelig for tilsynsmyndigheter. Velg
            &quot;Tilsynsansvarlig&quot; eller &quot;DMP-saksbehandler&quot; i rolle-velgeren øverst.
          </p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex border-b border-gray-200 mb-6">
        <button
          onClick={() => setAktivFane("losninger")}
        ...
        >
          ...
          <span className="ml-1 text-xs bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded-full">
            {MOCKDATA_BRUKSMELDINGER.length}
          </span>
          {bruksVarsler.ny > 0 && (
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
            <VarselKort
              ...
              onClick={() =>
                setAktivtFilter(aktivtFilter === "off-label" ? "alle" : "off-label")
              }
            />
            <VarselKort
              ...
              onClick={() =>
                setAktivtFilter(
                  aktivtFilter === "mangler-dmp" ? "alle" : "mangler-dmp"
                )
              }
            />
            <VarselKort
              ...
              onClick={() =>
                setAktivtFilter(
                  aktivtFilter === "mangler-dpia" ? "alle" : "mangler-dpia"
                )
              }
            />
            <VarselKort
              ...
              onClick={() =>
                setAktivtFilter(
                  aktivtFilter === "mangler-validering" ? "alle" : "mangler-validering"
                )
              }
            />
            <VarselKort
              ...
              onClick={() =>
                setAktivtFilter(aktivtFilter === "hoy-risiko" ? "alle" : "hoy-risiko")
              }
            />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
              ...
              <div className="space-y-3">
                ...
                <StatLinje
                  label="CE-merket"
                  verdi={MOCKDATA_LOSNINGER.filter((l) => l.ceMerket === "ja").length}
                />
                <StatLinje
                  label="MDR klasse IIb eller III"
                  verdi={
                    MOCKDATA_LOSNINGER.filter(
                      (l) => l.mdrKlasse === "IIb" || l.mdrKlasse === "III"
                    ).length
                  }
                />
                <StatLinje
                  label="LLM-baserte løsninger"
                  verdi={MOCKDATA_LOSNINGER.filter((l) => l.algoritmeType === "llm").length}
                />
                <StatLinje
                  label="Kontinuerlig lærende"
                  verdi={MOCKDATA_LOSNINGER.filter((l) => l.kontinuerligLaerende).length}
                />
                <StatLinje
                  label="Avvikshendelser rapportert"
                  verdi={
                    MOCKDATA_LOSNINGER.filter((l) => l.avvikshendelser !== "ingen").length
                  }
                />
                <StatLinje
                  label="Utenfor EØS-infrastruktur"
                  verdi={
                    MOCKDATA_LOSNINGER.filter((l) => l.norskInfrastruktur === "nei").length
                  }
                  farge="text-red-600"
                />
              </div>
                    ...
                    <tr key={l.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className="text-xs text-gray-600">
                          {l.aiActRisiko === "hoy-medisinsk" ||
                          l.aiActRisiko === "hoy-annex3" ? (
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`text-xs px-2 py-0.5 rounded ${
                            l.dpiaGjennomfort === "ja"
                              ? "bg-green-50 text-green-700"
                              : l.dpiaGjennomfort === "pagaar"
                              ? "bg-yellow-50 text-yellow-700"
      ...
      {/* ── FANE 2: Bruksmeldinger ── */}
      {aktivFane === "bruksmeldinger" && (
        <>
          {/* Varsel-dashboard */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            <VarselKort
              tittel="Nye (ubehandlede)"
              antall={bruksVarsler.ny}
              ikon={<ClipboardList size={18} />}
              farge="bg-blue-50 border-blue-200 text-blue-700"
              aktiv={bruksFilter === "ny"}
              onClick={() => setBruksFilter(bruksFilter === "ny" ? "alle" : "ny")}
            />
            <VarselKort
              tittel="Til oppfølging"
              antall={bruksVarsler.til_oppfolging}
              ikon={<Clock size={18} />}
              farge="bg-amber-50 border-amber-200 text-amber-700"
              aktiv={bruksFilter === "til-oppfolging"}
              onClick={() =>
                setBruksFilter(
                  bruksFilter === "til-oppfolging" ? "alle" : "til-oppfolging"
                )
              }
            />
            <VarselKort
              tittel="Avvist"
              antall={bruksVarsler.avvist}
              ikon={<XOctagon size={18} />}
              farge="bg-red-50 border-red-200 text-red-700"
              aktiv={bruksFilter === "avvist"}
              onClick={() =>
                setBruksFilter(bruksFilter === "avvist" ? "alle" : "avvist")
              }
            />
            <VarselKort
              tittel="Off-label / usikker"
              antall={bruksVarsler.off_label}
              ikon={<AlertTriangle size={18} />}
              farge="bg-orange-50 border-orange-200 text-orange-700"
              aktiv={false}
              onClick={() => {}}
          ...
          {bruksFilter !== "alle" && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4 flex items-center justify-between">
              <p className="text-sm text-amber-800">
                ...
                {BRUKSMELDING_STATUS_META[bruksFilter]?.etikett} · {filtrerteBruksmeldinger.length} meldinger
              </p>
              <button
                onClick={() => setBruksFilter("alle")}
                className="text-xs text-amber-700 hover:text-amber-900 font-medium"
              >
                Nullstill ✕
              </button>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              <h2 className="text-sm font-semibold text-gray-700">
                Innmeldte bruksmeldinger ({filtrerteBruksmeldinger.length})
              </h2>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                  ...
                  {filtrerteBruksmeldinger.map((b) => {
                    ...
                    return (
                      <tr key={b.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <p className="font-medium text-gray-900">{b.produktnavn}</p>
                          <p className="text-xs text-gray-400">{b.leverandor}</p>
                        </td>
                        <td className="px-4 py-3">
                          <p className="text-xs text-gray-700">
                            {b.helseforetak.split("(")[0].trim()}
                          </p>
                          <p className="text-xs text-gray-400">{b.klinikk}</p>
                        </td>
                        <td className="px-4 py-3">
                          <span
                            className={`text-xs px-2 py-0.5 rounded border ${
                              b.erInnenforGodkjenning === "ja"
                                ? "bg-green-50 text-green-700 border-green-200"
                                : b.erInnenforGodkjenning === "usikker"
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded border ${statusMeta.farge}`}
                          >
                            {statusMeta.ikon}
                            {statusMeta.etikett}
                          </span>
          ...
          <div className="mt-6 bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            <h3 className="text-sm font-semibold text-gray-700 mb-3">
              Siste tilsynskommentarer
            </h3>
            <div className="space-y-3">
              {MOCKDATA_BRUKSMELDINGER.filter((b) => b.tilsynskommentar).map((b) => (
                <div key={b.id} className="border-l-4 border-primary-300 pl-4 py-1">
                  ...
                  <p className="text-xs text-gray-600 mt-0.5">{b.tilsynskommentar}</p>
                  <p className="text-xs text-gray-400 mt-0.5">{formatDato(b.sistOppdatert)}</p>
                </div>
              ))}
            </div>
          </div>
...
function VarselKort({
  ...
  return (
    <button
      onClick={onClick}
      className={`rounded-xl border p-4 text-left transition-all ${farge} ${
        aktiv ? "ring-2 ring-offset-1 ring-gray-400 shadow-md" : "hover:shadow-sm"
      }`}
    >
      <div className="flex items-center gap-2 mb-1">
        {ikon}
